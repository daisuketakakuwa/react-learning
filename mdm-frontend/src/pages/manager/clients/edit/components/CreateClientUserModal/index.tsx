...
import { createSalesAccount } from '@/lib/api/APIHandler';
import { useState } from 'react';
import styles from './index.module.css';

const CreateClientUserModal = (props: { clientId: string; isOpen: boolean; onClose: () => void; callbackAfterTriggerAPI: () => void }) => {
    // 入力値
    ...
    const callCreateClientUserAPI = () => {
        if (!validateInput()) {
            createSalesAccount(email, password, username, props.clientId).then(() => {
                resetAllInput();
        ...
        >
            <>
                ...
                <div className={styles.btnWrapper}>
                    <Button onClick={callCreateClientUserAPI}>登録する</Button>
                </div>
            </>
        </Modal>
    );
